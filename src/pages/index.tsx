	...
	return (
		<div className={styles.column}>
			...
			<Droppable
				...
				mode="virtual"
				isDropDisabled={activeColumn ? !(activeColumn.acceptIds.includes(id) || id === activeColumn.id) : true}//是否能接受拖动
			>
				{(provided, snapshot) => (
					<div
						ref={provided.innerRef}
...
export default () => {
	...
	const [ activeColumn, setActiveColumn ] = useState<initialDataInferface | null>(null);
		...
		<DragDropContext onDragEnd={onDragEnd} onDragStart={onDragStart}>
			<div className={styles.container}>
				{data.map((column, index) => {
					return <Column columnIndex={index} key={column.id} activeColumn={activeColumn} column={column} />;
				})}
			</div>
		</DragDropContext>
	);
